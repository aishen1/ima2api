...
页面上的地址由服务端按下面的顺序推算，**始终带协议与端口**：

1. 配置里的 `server.public_base`（在配置页「地址不对？手动指定」里设置）
2. 请求的 `Host` 头（用浏览器访问时就是你输入的地址）
3. Host 缺失、或指向 `127.0.0.1` / `localhost`（本机开页面就会这样）时，
   自动换成本机**内网 IP**（从网卡枚举中优先取 `192.168.x` / `10.x` / `172.16-31.x`）

第 3 条很关键：在本机浏览器打开配置页拿到的是 `127.0.0.1`，
**别的设备根本连不上**，所以这种情况下会直接给出内网地址。
