...
页面上的地址**由浏览器自己算**，所以你在哪台设备上打开，就得到那台设备真能连通的完整地址（始终带协议与端口）：

1. 配置里的 `server.public_base`（在配置页「地址不对？手动指定」里设置，优先级最高）
2. 浏览器当前访问地址（`location`）—— 手机用 `http://192.168.9.102:8088` 打开，就显示这个地址
3. 上面拿到的是 `127.0.0.1` / `localhost`（在 NAS 本机开页面就会这样）时，
   改用服务端返回的内网 IP（从网卡枚举中优先取 `192.168.x` / `10.x` / `172.16-31.x`）

第 3 条很关键：在本机浏览器打开配置页拿到的是 `127.0.0.1`，**别的设备根本连不上**，
所以这种情况下会直接给出内网地址。

> 早期版本把「显示什么地址」交给服务端按请求 `Host` 头推算，一旦这一步拿到异常值，
> 页面就会显示成 `/v1` 这种半截内容。现在地址由浏览器端计算，
> 服务端返回值只在格式完整时才兜底采用，不会再出现半截地址。

页面上的地址与 Key 使用可断行的完整显示（不再横向裁切），
复制按钮在 `http://` 局域网下也能用（`navigator.clipboard` 只在安全上下文可用，
已自动回退到 `execCommand`）。

配置页标题右侧会显示版本标记（如 `v1.0.4`），用来一眼确认当前打开的是哪一版页面；
